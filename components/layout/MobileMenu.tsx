    ...
    <details className="group relative z-20 xl:hidden">
      <summary className="focus-ring flex size-10 cursor-pointer list-none items-center justify-center rounded-md [&::-webkit-details-marker]:hidden" aria-label="Open navigation menu">
        <span className="flex w-6 flex-col gap-1.5">
          <span className="h-0.5 rounded-full bg-navy transition group-open:translate-y-2 group-open:rotate-45" />
          <span className="h-0.5 rounded-full bg-navy transition group-open:opacity-0" />
          <span className="h-0.5 rounded-full bg-navy transition group-open:-translate-y-2 group-open:-rotate-45" />
        </span>
      </summary>

      <div className="absolute right-0 top-14 w-[min(16.5rem,calc(100vw-2rem))] overflow-hidden rounded-2xl border border-white/45 bg-[#F2A30F] p-3 text-navy shadow-[0_22px_70px_rgba(15,23,42,0.22)] backdrop-blur-xl">
        ...
        <nav aria-label="Mobile navigation" className="relative z-10 grid gap-1">
          {navItems.map((item) => (
